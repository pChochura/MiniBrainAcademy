		...
		if(intent != null) {
			if(intent.getBooleanExtra("open", false))
				startActivity(new Intent(this, SplashActivity.class)
						.putExtra("open", getResources().getString(R.string.schedule))
						.putExtra("id", intent.getStringExtra("id"))
						.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK));

			//Mark as read
			UUID eventId = UUID.fromString(intent.getStringExtra("id"));
			...
			if(currentUser != null) {
				FirebaseDatabase.getInstance().getReference("users").child(currentUser.getUid()).child("id").addListenerForSingleValueEvent(new ValueEventListener() {
					@Override public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
						...
						FirebaseDatabase.getInstance().getReference("events").child(eventId.toString()).addListenerForSingleValueEvent(new ValueEventListener() {
							@Override public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
								Event e = Event.loadEvent(dataSnapshot);
								if(e.getId() != null && e.getReadBy().isEmpty() || Readable.isNotRead(userId, e)) {
									e.getReadBy().add(userId);
									GlobalData.saveEvent(e, () -> {
